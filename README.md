...
Run the numbered steps from the repo root. Layout:
- `setup/`: install steps 01-07 and 10 (move the model cache to NVMe), `vllm-serve.sh` / `stop-vllm.sh`,
  the HPU-vs-CPU numeric checks (`hpu-size.py`, `hpu-bisect.py`) and the Debian driver patch.
- `experiments/`: benchmark and load-test scripts 08-22, in the order they were run.
- `results/`: result tables (`*.md`) and console output of each experiment (`*.out`). The write-up is [FINDINGS.md](FINDINGS.md).
- `logs/`: raw logs from setup, benchmarks (`bench/`), load tests (`office/`) and server starts (`serve/`; `serve-FAILED-*` did not start).
...
- After reboot: `bash setup/07-verify.sh` (all rel err ~1e-6 => fixed), then `bash experiments/08-bench.sh <model> <tp>`.
