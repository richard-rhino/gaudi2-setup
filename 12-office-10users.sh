...
# Usage: TP=8 EXTRA="--enable-expert-parallel" bash ~/setup/12-office-10users.sh <model> [max_model_len=131072]
MODEL="${1:?model}"; MAXLEN="${2:-131072}"; TP="${TP:-8}"; EXTRA="${EXTRA:-}"; CARDS="${CARDS:-all}"; C="${C:-vllm-gaudi-stable}"; PORT=8000
NAME=$(echo "$MODEL" | tr '/' '_')-tp${TP}-$(echo "$EXTRA" | tr -dc 'a-z0-9' | cut -c1-10)-ctx$MAXLEN-mem${GPU_MEM:-0.85}; OUT=~/setup/office10-$NAME.md
...
bench() { local name=$1 n=$2 conc=$3 in=$4 out=$5
  ...
  docker exec $C bash -c "cd /models && vllm bench serve --backend vllm --host 127.0.0.1 --port $PORT --model '$MODEL' --served-model-name office \
     ...
     --ignore-eos --percentile-metrics ttft,tpot,itl,e2el --metric-percentiles 50,90,99 --seed 7 --disable-tqdm" > ~/setup/office10-$name-$NAME.log 2>&1
  local L=~/setup/office10-$name-$NAME.log; g() { grep -E "$1" "$L" | tail -1 | awk '{print $NF}'; }
